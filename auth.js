(() => {
  ...
  function setMessage(text = '', type = '') {
    if (!message) return;
    I18N.text(message, () => I18N.error(text, ""));
  ...
  function setView(name) {
    ...
    if (title) I18N.text(title, labels[name]?.[0] || 'Аккаунт');
    if (subtitle) I18N.text(subtitle, labels[name]?.[1] || '');
  }
  ...
  function applyAuthUi() {
    ...
    if (user) {
      ...
      if (accountButton) {
        I18N.text(accountButton, 'Кіру / Тіркелу');
  ...
  async function signIn() {
    ...
    if (error) return setMessage(error || 'Кіру мүмкін болмады.', 'error');
  ...
  async function signUp() {
    ...
    if (error) return setMessage(error || 'Тіркелу мүмкін болмады.', 'error');
  ...
  async function sendReset() {
    ...
    if (error) return setMessage(error || 'Хат жіберілмеді.', 'error');
  ...
  async function setNewPassword() {
    ...
    if (error) return setMessage(error || 'Құпиясөзді өзгерту мүмкін болмады.', 'error');
    recoveryMode = false;
  ...
  async function updateAccountName() {
    ...
    if (error) return setMessage(error || 'Атты өзгерту мүмкін болмады.', 'error');
