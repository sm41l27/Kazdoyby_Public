...
module.exports=function({io,rooms,requireSocketUser,avatars,tournaments,roomStatus,emitStatus,maybeStart}){
 ...
 async function move(room,target,seat){
  ...
  if(seat==='spectator'){
   slots[source]=null;profiles[source]=null;spectators.add(target);if(room.tournament)types[source]='ai-medium';
  }else{
  ...
  if(room.tournament){
   ...
   for(const c of COLORS)types[c]=seats[c]?'human':'ai-medium';
