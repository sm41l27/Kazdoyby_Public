...
function removeSocketFromRoom(socket, { announce = true } = {}) {
  ...
  if(!remaining.length && !room.spectators?.size){
    clearRoomTimer(room);queueDb(room,()=>abandonDbMatch(room));rooms.delete(code);return {room:null,color:leavingColor};
  }
...
function addObserver(socket,room,profile=null){
  ...
  if(room.started){
    socket.emit('game-start',{...roomStatus(room)});
    if(room.state)socket.emit('game-state',{state:room.state,winner:room.winner,endReason:room.endReason,timeResult:room.timeResult});
...
require('./room-control')({io,rooms,requireSocketUser,avatars,tournaments,roomStatus,emitStatus,maybeStart,profileDto});
...
io.on('connection', (socket) => {
  ...
  socket.on('resign-room', () => {
    ...
    tournaments.complete(room);
    io.to(room.code).emit('game-state', {state:room.state,winner:room.winner,endReason:room.endReason});
    emitStatus(room);
  });
