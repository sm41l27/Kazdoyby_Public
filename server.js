...
const avatars = require('./avatars-server')({app,supabaseRest,enabled:DB_ENABLED});
...
function profileDto(row) {
  ...
  return {
    ...
    avatar: row.avatar || {type:'icon',value:'pawn'},
...
async function supabaseRest(table, { method = 'GET', query = '', body = undefined, prefer = '' } = {}) {
  ...
  // Legacy JWT service keys use Bearer auth; new sb_secret keys use the API gateway.
...
async function ensurePlayerRecord(authUser, rawProfile = {}) { return avatars.attach(await ensurePlayerRecordBase(authUser, rawProfile)); }

async function ensurePlayerRecordBase(authUser, rawProfile = {}) {
...
async function getPlayerByAuthUser(authUser) {
  ...
  return avatars.attach(Array.isArray(found) && found[0] ? found[0] : null);
}
...
async function loadHomeData(authUser, rawClientId = '') {
  ...
  const [matchRows, participantRows] = await Promise.all([
    supabaseRest('matches', {
      method: 'GET',
      query: `id=in.${inFilter}&select=id,room_code,game_mode,rating_mode,winner,status,started_at,finished_at&order=id.desc`
    }),
  ...
  for (const ownRow of own) {
    ...
    result.push({
      ...
      ratingMode: match.rating_mode || 'rated',
...
async function updatePlayerStatistics(room, winners, isDraw = false) {
  ...
  const ratingEligible = (room.ratingMode || 'rated') === 'rated' && humanParticipants.length >= 2;
  ...
  for (const color of humanParticipants) {
    ...
    // Unrated results must not write the rating column, including during concurrent rated games.
    if (ratingDelta !== 0) next.rating = Math.max(100, Number(current.rating ?? DEFAULT_PROFILE.rating) + ratingDelta);
    ...
    if (socketId) {
      io.to(socketId).emit('profile-updated', {
        ...
        ratingDelta,
        ratingMode: room.ratingMode || 'rated',
...
async function ensureDbMatch(room) {
  ...
  const inserted = await supabaseRest('matches', {
    ...
    body: {
      ...
      rating_mode: room.ratingMode || 'rated',
      winner: null,
      status: 'playing'
    },
    prefer: 'return=representation'
...
async function finishRoomByTime(room) {
  if (!room || !rooms.has(room.code) || !room.started || room.winner || room.engineError) return;
  clearRoomTimer(room);
...
function roomStatus(room) {
  const members = [...Object.values(room.slots).filter(Boolean), ...(room.spectators || [])].map(id => {
    const member = io.sockets.sockets.get(id);if(!member)return null;
    return {id, userId:member.data.authUserId || null, name:member.data.playerProfile?.name || '', avatar:member.data.playerProfile?.avatar || {type:'icon',value:'pawn'}, color:COLORS.find(c=>room.slots[c]===id) || null, organizer:member.data.authUserId===room.ownerId};
  }).filter(Boolean);
  const playerNames = {};
  ...
  return {
    ...
    authoritative: Boolean(room.tournament),
    revision: room.revision || 0,
    serverNow: Date.now(),
    engineError: room.engineError || null,
    members,
    gameMode: room.gameMode,
    ratingMode: room.ratingMode || 'rated',
...
function maybeStart(room) {
  ...
  if (!room.manualStart && !room.started && needed > 0 && connectedHumans(room) === needed) {
    room.started = true;
    ...
    if(room.tournament) tournamentPlay.initialize(room);
    ...
    io.to(room.code).emit('game-start', {
      ...
      ...(room.tournament?tournamentPlay.snapshot(room):{})
    });
    emitStatus(room);
  }
}
...
function authorizedToSubmit(room, socket) {
  if (socket.data.spectator) return false;
...
function removeSocketFromRoom(socket, { announce = true } = {}) {
  const code=socket.data.roomCode,room=rooms.get(code),leavingColor=socket.data.color||null;
  socket.leave(code);socket.data.roomCode=null;socket.data.color=null;socket.data.spectator=false;
  if(!room)return null;
  room.spectators?.delete(socket.id);
  for(const color of COLORS)if(room.slots[color]===socket.id)room.slots[color]=null;
  const remaining=COLORS.map(c=>room.slots[c]).filter(Boolean);
  if(!remaining.length && !room.spectators?.size){
    clearRoomTimer(room);clearTimeout(room.botTimer);queueDb(room,()=>abandonDbMatch(room));rooms.delete(code);return {room:null,color:leavingColor};
  }
  if(room.hostSocketId===socket.id){room.hostSocketId=remaining[0]||null;if(room.hostSocketId)io.to(room.hostSocketId).emit('host-changed',{isHost:true});}
  emitStatus(room);if(announce&&leavingColor)io.to(code).emit('player-left',{color:leavingColor});
  return {room,color:leavingColor};
}

function addObserver(socket,room,profile=null){
  room.spectators.add(socket.id);socket.join(room.code);
  socket.data.roomCode=room.code;socket.data.color=null;socket.data.spectator=true;
  socket.data.playerProfile=profile;socket.data.authUserId=profile?.auth_user_id||null;
  socket.emit('room-spectating',{code:room.code,color:null,isHost:false,spectator:true,isOrganizer:socket.data.authUserId===room.ownerId,gameMode:room.gameMode,timeControlSec:room.timeControlSec,playerTypes:room.playerTypes,profile:profileDto(profile),status:roomStatus(room)});
  if(room.started){
    socket.emit('game-start',{...roomStatus(room),...(room.tournament?tournamentPlay.snapshot(room):{})});
    if(room.state)socket.emit('game-state',room.tournament?tournamentPlay.snapshot(room):{state:room.state,winner:room.winner,endReason:room.endReason,timeResult:room.timeResult});
    if(room.timeResult)socket.emit('time-expired',{deadlineAt:room.deadlineAt,winner:room.winner,result:room.timeResult});
  }
  emitStatus(room);
}
...
function changeRoomCode(room, newCode) {
  ...
  const socketIds = [...COLORS.map(c => room.slots[c]).filter(Boolean), ...(room.spectators || [])];
...
function createRoomRecord({code, gameMode='teams', timeControlSec=600, playerTypes, ratingMode='rated', allowSpectators=true, tournament=null, ownerId=null, manualStart=true}) {
  return {
      code,
      gameMode,
      timeControlSec,
...
require('./social-server')({ io, rooms, fetchAuthUser, supabaseRest, enabled: DB_ENABLED && AUTH_ENABLED, avatars });

const tournaments = require('./tournament-server')({io, rooms, fetchAuthUser, supabaseRest, enabled:DB_ENABLED && AUTH_ENABLED, createRoomRecord, makeRoomCode, avatars,
  closeTournamentRooms(id){for(const room of rooms.values())if(room.tournament?.id===id&&!room.winner){
    clearRoomTimer(room);clearTimeout(room.botTimer);room.engineError=null;room.winner='draw';room.endReason='cancelled';
    queueDb(room,()=>abandonDbMatch(room));io.to(room.code).emit('game-state',tournamentPlay.snapshot(room));emitStatus(room);
  }}
});
const tournamentPlay=require('./tournament-play')({io,rooms,requireSocketUser,emitStatus,
  persist(room,state){queueDb(room,async()=>{await ensureDbMatch(room);await saveNewMoves(room,state);if(room.winner)await finishDbMatch(room,room.winner);});},
  finishByTime:finishRoomByTime,
  finishNormal(room){clearRoomTimer(room);tournaments.complete(room);},
  pauseTimer:clearRoomTimer,resumeTimer:scheduleRoomTimer
});
server.on('close',()=>tournamentPlay.close());
require('./room-control')({io,rooms,requireSocketUser,avatars,tournaments,roomStatus,emitStatus,maybeStart});
...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', async (payload = {}) => {
    ...
    const room = createRoomRecord({code, gameMode, timeControlSec, playerTypes, ownerId:authUser.id, ratingMode:payload.ratingMode === 'friendly' ? 'friendly' : 'rated', allowSpectators:payload.allowSpectators !== false});
  ...
  socket.on('join-room', async (payload = {}) => {
    ...
    if (socket.data.roomCode || rooms.get(code) !== room) return;
    if (room.tournament && !room.tournament.pair.includes(authUser.id)) { socket.emit('room-error', 'arena.error.match_only'); return; }
    if (room.tournament) {
      try { await tournaments.authorizeJoin(authUser.id, room); } catch (_) { socket.emit('room-error', 'arena.error.closed'); return; }
      if (socket.data.roomCode || rooms.get(code) !== room) return;
    }
    if (room.editing) { socket.emit('room-error','room.error.busy');return; }
    if (room.tournament && !Object.values(room.tournament.players).includes(authUser.id)) {addObserver(socket,room,profile);return;}
    const duplicateAccount = COLORS.some(c => room.slots[c] && room.profiles?.[c]?.auth_user_id === authUser.id);
    if (duplicateAccount) {
      socket.emit('room-error', 'Бұл аккаунт осы бөлмеде әлдеқашан бар. Бір аккаунтпен екі орын алуға болмайды.');
      return;
    }

    const reserved = room.tournament ? Object.keys(room.tournament.players).find(c => room.tournament.players[c] === authUser.id) : COLORS.find(c => room.profiles?.[c]?.auth_user_id === authUser.id && !room.slots[c]);
    // Once play begins only the original player may reconnect to a vacant seat.
    if (room.started && !reserved) { socket.emit('room-error', 'room.error.started'); return; }
    const color = reserved && !room.slots[reserved] ? (room.slots[reserved] = socket.id, reserved) : room.tournament ? null : assignHumanSlot(room, socket.id);
    if (!color) {
    ...
    socket.emit('room-joined', {
      ...
      isOrganizer: authUser.id === room.ownerId,
      gameMode: room.gameMode,
      timeControlSec: room.timeControlSec,
      deadlineAt: room.deadlineAt,
      playerTypes: room.playerTypes,
    ...
    if (room.started) {
      ...
      if (room.state) {
        setTimeout(() => {
          socket.emit('game-state',room.tournament?tournamentPlay.snapshot(room):{state:room.state,winner:room.winner,endReason:room.endReason||null,timeResult:room.timeResult||null});
        }, 150);
      }
      if (room.timeResult) {
  ...
  socket.on('watch-room', async (payload = {}) => {
    if (socket.data.roomCode) { socket.emit('room-error','arena.error.leave_room');return; }
    const code=normalizeRoomCode(payload.code),room=rooms.get(code);
    if(!room){socket.emit('room-error','arena.error.room_unavailable');return;}
    let profile=null;
    try{if(socket.handshake.auth?.token){const user=await requireSocketUser(socket);profile=await ensurePlayerRecord(user,payload.profile||{});}}catch{socket.emit('room-error','room.error.auth_required');return;}
    if(socket.data.roomCode||rooms.get(code)!==room)return;
    if(room.allowSpectators===false&&profile?.auth_user_id!==room.ownerId){socket.emit('room-error','arena.error.watch_disabled');return;}
    if(room.spectators.size>=100){socket.emit('room-error','arena.error.watch_full');return;}
    addObserver(socket,room,profile);
  });

  socket.on('resign-room', () => {
    const room = rooms.get(socket.data.roomCode), color = socket.data.color;
    if (!room?.tournament || !room.started || room.winner || socket.data.spectator || room.slots[color] !== socket.id) return;
    room.winner = ['blue','black'].includes(color) ? 'red-white' : 'blue-black';
    room.endReason = 'resign'; clearRoomTimer(room);
    queueDb(room, () => finishDbMatch(room, room.winner));
    tournaments.complete(room);
    io.to(room.code).emit('game-state', {...tournamentPlay.snapshot(room)});
    emitStatus(room);
  });
  ...
  socket.on('change-room-code', (payload = {}) => {
    ...
    if (room.ownerId !== socket.data.authUserId) {
    ...
    if (room.tournament) { socket.emit('room-error', 'arena.error.tournament_locked'); return; }
  ...
  socket.on('game-state', (payload = {}) => {
    ...
    if(room.tournament){socket.emit('room-error','play.serverTurns');return;}
    ...
    if (room.winner) { clearRoomTimer(room); tournaments.complete(room); emitStatus(room); }
  ...
  socket.on('restart-room', () => {
    const code = socket.data.roomCode;
    const room = code && rooms.get(code);
    if (!room || room.ownerId !== socket.data.authUserId) return;
    if (room.tournament) { socket.emit('room-error', 'arena.error.tournament_locked'); return; }
...
module.exports = {server, io, rooms, updatePlayerStatistics, createRoomRecord, roomStatus, tournaments};
