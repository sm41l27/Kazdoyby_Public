...
module.exports = function ({io, rooms, fetchAuthUser, supabaseRest, enabled, createRoomRecord, makeRoomCode, avatars}) {
  const uuid = value => typeof value === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);
  const actions = new Set(['list','create','get','join','leave','start','cancel','enter','manage']);
  ...
  async function createMatch(user,m,t,manage=false){
    ...
    const playerTypes=Object.fromEntries(['blue','black','red','white'].map(c=>[c,seats[c]?'human':'ai-medium']));
    const room=createRoomRecord({code,gameMode:'teams',timeControlSec:t.time_control,ratingMode:'tournament',allowSpectators:true,ownerId:t.owner_id,playerTypes,tournament:{id:t.id,match:m.id,pair:[m.player1,m.player2],players:seats}});
